...
def update_all_paytimes():
    conn = get_db_connection()
    read_cursor = conn.cursor(dictionary=True)
    write_cursor = conn.cursor()

    try:
        print("Fetching max pay_time for all paying orders...")
        
        # We group by subscription_key to find the LATEST pay_time for each subscription.
        # This is equivalent to processing "in order" and keeping the final one.
        query = """
        ...
        count = len(updates)
        print(f"Found {count} subscriptions with payments to update.")
        ...
        for row in updates:
            ...
            if len(batch_data) >= batch_size:
                ...
                print(f"  Processed {updated_count} keys...")
                batch_data = []
        ...
        end_ts = time.time()
        print(f"Update Complete. Processed {updated_count} subscription keys.")
